...
G4AssemblyVolume *PHG4TpcEndCapDetector::ConstructEndCapAssembly()
{
  ...
  //instead of building this layer-by-layer, we build a single block corresponding to all the gem layers that were previously handled in this fashion:
  totalThickness *= n_GEM_layers; //we have n_GEM_layers of thickness totalThickness each.
...
G4LogicalVolume *PHG4TpcEndCapDetector ::AddLayer(  //
...
void PHG4TpcEndCapDetector::ConstructGemFrames(G4LogicalVolume *gemvol, float thickness)
{
 ...
 name_base = boost::str(boost::format("%1%_Layer_%2%") % GetName() % "R2_R3_Frames");
  ...
  G4VSolid *allFrames=new G4UnionSolid(name_base,unionsolid,spar,rm,G4ThreeVector(0.,0.,0.));
  ...
  //G4VPhysicalVolume *tpc_gem_frames =
