...
class AnnularFieldSim
{
 ...
 public:
  ...
  void loadEfield(const std::string &filename, const std::string &treename, int zsign = 1, TVector3 *fieldOrigin=nullptr, float eulerPhi=0., float eulerTheta=0., float eulerPsi=0.);
  void loadBfield(const std::string &filename, const std::string &treename, TVector3 *fieldOrigin=nullptr, float eulerPhi=0., float eulerTheta=0., float eulerPsi=0.);
  void load3dBfield(const std::string &filename, const std::string &treename, int zsign = 1, float scale = 1.0, TVector3 *fieldOrigin=nullptr, float eulerPhi=0, float eulerTheta=0, float eulerPsi=0);

  void loadField(MultiArray<TVector3> **field, TTree *source, float *rptr, float *phiptr, float *zptr, float *frptr, float *fphiptr, float *fzptr, float fieldunit, int zsign, TVector3 *fieldOrigin, float eulerPhi, float eulerTheta, float eulerPsi); //load field vector data from the source TTree, assuming the field coordinates are centered at a position fieldOrigin in the TPC coordaintes, and hence all coordinates in the source should be transformed into TPC coordinates via x_tpc=rotation(x_f)+fieldOrigin, where the rotation is defined by the euler angles according to EulerAngleX in the root reference guide.
